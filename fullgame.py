...
def main():
    ...
    with Run("fullgame", log=P) as run:
        ...
        # Event checkpointing is off by default. It writes a 17.9 KB savestate per major
        # acquisition, and the index is the durable half - see .gitignore for why the blobs
        # are not tracked. A run that does not ask for it behaves exactly as before.
        if os.environ.get("ZELDA_EVENT_CKPT") == "1":
            run.watch_events()

        # --list-events comes before any resume decision, so it lists rather than resuming
        # into a run when a checkpoint happens to exist. Read from the index, not the
        # checkpoint directory, because the index is what says what each one is *for*.
        if "--list-events" in args:
            rows = run.load_event_index()
            if not rows:
                P("--list-events: no index at logs/fullgame.checkpoints.json. Run with "
                  "ZELDA_EVENT_CKPT=1 first, or see issue #4.")
                return
            P(f"{len(rows)} event checkpoints:")
            for e in rows:
                P(f"  {e['key']:46s} frame {e['frame']:>7}  {e['name']}"
                  + ("  (approximate frame)" if e.get("approximate") else ""))
            return
        ...
        skip = set()
        if "--from-event" in args:
            # An event checkpoint is an ordinary checkpoint reached by its index key, so the
            # input prefix and the list_hash guard both come along unchanged. The index is
            # loaded first so resume_event can name what it is resuming from.
            key = args[args.index("--from-event") + 1]
            run.event_index = run.load_event_index()
            if not run.event_index:
                P(f"--from-event: no index at logs/fullgame.checkpoints.json")
                return
            if run.resume_event(key) is None:
                P(f"no event checkpoint '{key}'. Try --list-events.")
                return
            skip = set(run.done)
        ...
        if os.environ.get("ZELDA_EVENT_CKPT") == "1" and run._spans:
            P(f"note: {run._spans} steps advanced more than one frame, so any event attributed to "
              f"one has an approximate frame in the index. The checkpoints themselves are valid.")
        run.finish(comment="AI plays The Legend of Zelda. Generated by the harness.")
